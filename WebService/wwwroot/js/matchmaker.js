; (function () {
    ...
    mk.controllers.Network = function (options) {
        ...
        this._transport = options.transport || this.Transports.hub_connection;
    };
    ...
    mk.controllers.Network.prototype._addHandlers = function () {
        var pressed = {},
            self = this,
            m = this.Messages,
            f = this.fighters[this._player];
    ...
    mk.controllers.Network.prototype._addConnectionHandlers = function () {
        var opponent = this.fighters[+!this._player],
            f = this.fighters[this._player],
            m = this.Messages,
            self = this;

        //this._transport.connection.on(m.STATE, gameState => {
            // todo: get moves from state
            //var move = self._getMove(pressed, mk.controllers.keys, self._player);
            //self._moveFighter(f, move);
            //move = self._getMove(pressed, mk.controllers.keys, +!self._player);
            //self._moveFighter(f, move);

            // todo: set life, update life on 2000ms
            //f.setLife(f.getLife());
            //opponent.setLife(opponent.getLife());

            // todo: set position if jumping, update position on 500ms
            //if (f.isJumping()) {
            //    f.setX(f.getX());
            //    f.setY(f.getY());
            //}
        ...
        this._transport.connection.on(m.STATE_MOVE, move => {
            opponent.setMove(move);
        });

        this._transport.connection.on(m.LIFE_UPDATE, data => {
            opponent.setLife(data);
        });

        this._transport.connection.on(m.POSITION_UPDATE, (x, y) => {
            opponent.setX(x);
            opponent.setY(y);
        });
        ...
        this._transport.connection.on(m.END_GAME, () => {
            self._transport.connection.stop();
            clearInterval(self._lifeUpdateTimer);
            clearInterval(self._positionUpdateTimer);
        });
    };
    ...
    mk.controllers.Network.prototype.fighterDead = function (fighter) {
        var opponent = this.getOpponent(fighter),
            callback = this._callbacks[mk.callbacks.GAME_END],
            m = this.Messages;
        opponent.getMove().stop();
    ...
    mk.arenas.Arena.prototype._drawArena = function () {
        ...
        if (this.texture) {
            ...
            var src = location.substring(0, location.length - 15) + conf.IMAGES + conf.ARENAS + this.arena + '/arena.png';
    ...
    mk.moves.Move.prototype._getImageUrl = function (id, ownerOrientation) {
        var conf = mk.config;
        var location = window.location.href;
        var src = location.substring(0, location.length - 15) + conf.IMAGES +
            conf.FIGHTERS +
            this.owner.getName() + '/' +
            ownerOrientation + '/' +
            this.type + '/' +
...
//    li.textContent = "[MATCH STARTED] ActorId long " + mLong + " player: " + playerId;
